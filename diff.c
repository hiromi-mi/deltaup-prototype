...
int main(int argc, const char **argv) {
   ...
   //                         max((long)orignum - wind_cnt * (long)WINDOW_CHARS,
   //                         1));
   //      size_t j_max = min(WINDOW_SIZE,
   //                         max((long)newnum - wind_cnt * (long)WINDOW_CHARS,
   //                         1));
   //
   ...
   //            costs[DELETE] =
   //                table[i - 1][j] + 1 + (act[i - 1][j] != DELETE) * 2; //
   //                DELETE
   //            costs[INSERT] =
   //                table[i][j - 1] + 1 + (act[i][j - 1] != INSERT) * 2; //
   //                INSERT
   //            costs[SUBSTITUTE] =
   //                table[i - 1][j - 1] + 3 +
   ...
   //               if (buf_index == 0 || buffer_act[buf_index - 1] !=
   //               SUBSTITUTE ||
   ...
   //            case ADD:
   //            case DELETE:
   //               buffer_index[k] = (buffer_index[k] + 1 -
   //               buffer_char_len[k]); break;
   //            default:
   //               break;
   //         }
   //
   ...
   //               }
   //               for (int i = 0; i < buffer_char_len[k]; i++) {
   //                  buffer_chars_add[k][buffer_char_len[k] - i - 1] =
   //                  temp[i];
   //               }
   //               break;
   //            default:
   ...
   //                stdout);
   //         fwrite(&buffer_char_len[k], sizeof(buffer_index_delta[k]), 1,
   //         stdout);
   //
